...
import { BriefcaseBusiness, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Clock3, GitBranch, Hammer, Landmark, PhoneCall, RefreshCw, ShieldCheck, UserRound, UsersRound, type LucideIcon } from "lucide-react";
...
type CalendarKind = "handlowiec" | "monter" | "kierownik" | "ksiegowosc" | "internal";
...
const calendarKinds: Array<{
    ...
    { key: "handlowiec", role: "handlowiec", labelPl: "Handlowcy", icon: BriefcaseBusiness },
    { key: "monter", role: "monter", labelPl: "Monterzy", icon: Hammer },
    { key: "kierownik", role: "kierownik", labelPl: "Kierownicy", icon: UsersRound },
    { key: "ksiegowosc", role: "ksiegowosc", labelPl: "Księgowość", icon: Landmark },
    { key: "internal", labelPl: "Wewnętrzne", icon: ShieldCheck }
];
const hierarchyOrder: UserRole[] = ["owner", "admin", "kierownik", "handlowiec", "ksiegowosc", "logistyk", "monter"];
...
function isHeldMeeting(event: CalendarEvent) {
    if (event.type !== "meeting" || !event.lead)
        return false;
    if (event.lead.status === "Po spotkaniu" || event.lead.status === "Umowa")
        return true;
...
function allowedKinds(role: UserRole) {
    if (isSystemWide(role))
        return calendarKinds;
    if (role === "kierownik") {
        return calendarKinds.filter((kind) => ["handlowiec", "kierownik", "internal"].includes(kind.key));
    }
    if (role === "handlowiec")
        return calendarKinds.filter((kind) => kind.key === "handlowiec");
    if (role === "ksiegowosc")
        return calendarKinds.filter((kind) => ["ksiegowosc", "internal"].includes(kind.key));
    if (role === "logistyk" || role === "monter")
        return calendarKinds.filter((kind) => ["monter", "internal"].includes(kind.key));
...
function EventRow({ event }: {
    ...
    const content = (<>
      <span className={`flex h-10 w-10 items-center justify-center rounded-lg ${isMeeting ? "bg-leaf text-white" : isCallback ? "bg-solar text-ink" : "bg-sky/10 text-sky"}`}>
        {isMeeting ? (<CalendarDays className="h-5 w-5" aria-hidden="true"/>) : isCallback ? (<PhoneCall className="h-5 w-5" aria-hidden="true"/>) : (<UsersRound className="h-5 w-5" aria-hidden="true"/>)}
      </span>
      <span>
    ...
    const className = `grid gap-2 rounded-md border p-3 text-sm transition hover:border-ink sm:grid-cols-[auto_1fr_auto] ${isMeeting ? "border-leaf/20 bg-leaf/10" : isCallback ? "border-solar/30 bg-solar/10" : "border-sky/20 bg-sky/10"}`;
    if (event.leadId) {
        return (<Link href={`/leads/${event.leadId}`} className={className}>
        {content}
      </Link>);
    }
...
export default function CalendarPage() {
    const { loading, profile } = useAuth(["owner", "admin", "kierownik", "handlowiec", "ksiegowosc", "logistyk", "monter"]);
    const locale = "pl-PL";
    ...
    const [calendarEventsUnavailable, setCalendarEventsUnavailable] = useState(false);
    const kindOptions = useMemo(() => (profile ? allowedKinds(profile.role) : []), [profile?.role]);
    const visibleUsers = useMemo(() => (profile ? visibleProfilesFor(profile, users, calendarKind) : []), [calendarKind, profile, users]);
    ...
    async function loadUsers() {
        ...
        const { data } = await supabase
            .from("profiles")
            .select("*")
            .in("role", ["owner", "admin", "kierownik", "handlowiec", "ksiegowosc", "logistyk", "monter", "sales", "manager"])
    ...
    async function loadInternalEvents(owners: Profile[]) {
        ...
        if (dbError) {
            setCalendarEventsUnavailable(true);
            return [];
        }
        const rows = (data || []) as CalendarDbEvent[];
        if (rows.length === 0) {
            setCalendarEventsUnavailable(false);
            return [];
        }
        setCalendarEventsUnavailable(false);
    ...
    const visibleByRole = profile
        ? hierarchyOrder
            .map((role) => ({
            role,
            users: users.filter((user) => visibleProfilesFor(profile, users, role === "handlowiec"
                ? "handlowiec"
                : role === "kierownik"
                    ? "kierownik"
                    : role === "ksiegowosc"
                        ...
                        : role === "monter" || role === "logistyk"
                            ? "monter"
                            : "internal").some((visible) => visible.id === user.id) && normalizeRole(user.role, user.email) === role)
        }))
    ...
    return (<AppShell profile={profile}>
      <div className="grid gap-5">
        <PageHeader title={"Kalendarz"} description={"Kalendarze działów, kalendarze użytkowników i spotkania wewnętrzne z widocznością według hierarchii."} actions={<div className="flex flex-wrap gap-2">
            <select className="field w-full sm:w-72" value={selectedUserId} onChange={(event) => setSelectedUserId(event.target.value)} disabled={visibleUsers.length <= 1}>
              <option value="">{"Wszyscy widoczni użytkownicy"}</option>
              {visibleUsers.map((person) => (<option key={person.id} value={person.id}>
                  {person.full_name}
                </option>))}
            </select>
        ...
        <section className="grid gap-2 md:grid-cols-5">
          {kindOptions.map((kind) => {
            ...
            return (<button key={kind.key} type="button" onClick={() => setCalendarKind(kind.key)} className={`flex min-h-14 items-center justify-center gap-2 rounded-lg border px-3 text-sm font-black transition ${active ? "border-ink bg-ink text-white" : "border-line bg-white text-ink hover:border-ink"}`}>
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <div className="app-card">
            ...
            <div className="mt-1 text-sm text-muted">{"spotkania i wewnętrzne"}</div>
          </div>
          <div className="app-card">
        ...
        <section className="app-card">
          <SectionHeader icon={GitBranch} title={"Mapa hierarchii"} description={"Widoczni użytkownicy wynikają z uprawnień roli i przypisania przełożonych."} tone="sky"/>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {visibleByRole.map((group) => (<div key={group.role} className="rounded-lg border border-line bg-[#f9fbfd] p-3">
                <div className="text-sm font-black text-ink">{ROLE_LABELS[group.role]}</div>
                <div className="mt-3 grid gap-2">
                  {group.users.map((user) => (<div key={user.id} className="rounded-md border border-line bg-white px-3 py-2 text-sm">
        ...
        <section className="app-card">
          ...
          {calendarEventsUnavailable ? (<Alert tone="info" className="mb-4">
              {"Wpisy kalendarzy wewnętrznych pojawią się po włączeniu tabeli calendar_events."}
            </Alert>) : null}

          {error ? (<Alert tone="danger" className="mb-4">
              {error}
            </Alert>) : null}

          <div className="grid grid-cols-7 border-l border-t border-line text-xs font-bold uppercase tracking-wide text-muted">
            {(["Pon", "Wt", "Sr", "Czw", "Pt", "Sob", "Nd"]).map((item) => (<div key={item} className="border-b border-r border-line bg-[#f9fbfd] p-2">
                {item}
              </div>))}
          </div>
          <div className="grid grid-cols-7 border-l border-line">
            ...
            return (<button key={key} type="button" onClick={() => setSelectedDay(key)} className={`min-h-24 border-b border-r border-line p-2 text-left transition hover:bg-[#fbfcfe] sm:min-h-28 ${isCurrentMonth ? "bg-white" : "bg-[#f9fbfd] text-muted"} ${key === todayKey ? "ring-2 ring-inset ring-solar" : ""} ${isSelected ? "bg-sky/5" : ""}`}>
                  <span className="mb-2 block text-sm font-black">{day.getDate()}</span>
                  <span className="grid gap-1">
                    {dayMeetings.length > 0 ? (<span className="inline-flex w-fit items-center gap-1 rounded-full bg-leaf/10 px-2 py-1 text-[11px] font-black text-leaf">
                        <CalendarDays className="h-3 w-3" aria-hidden="true"/>
                        {dayMeetings.length}
                      </span>) : null}
                    {dayCallbacks.length > 0 ? (<span className="inline-flex w-fit items-center gap-1 rounded-full bg-solar/20 px-2 py-1 text-[11px] font-black text-[#8a5a00]">
                        <PhoneCall className="h-3 w-3" aria-hidden="true"/>
                        {dayCallbacks.length}
                      </span>) : null}
                    {dayInternal.length > 0 ? (<span className="inline-flex w-fit items-center gap-1 rounded-full bg-sky/10 px-2 py-1 text-[11px] font-black text-sky">
        ...
        <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="app-card">
            ...
            <div className="mt-3 grid gap-2">
              {events.slice(0, 12).map((event) => (<EventRow key={event.id} event={event}/>))}
              {!busy && events.length === 0 ? (<EmptyState title={"Brak zaplanowanych działań"} description={"Ten miesiąc nie ma widocznych wpisów kalendarza."}/>) : null}
            </div>
          </div>
        </section>
      </div>
