...
export async function POST(request: Request) {
  ...
  const payload = {
    ...
    additional_notes: text(body,"additional_notes") || null, created_by: profile.id, crm_environment: profile.crm_environment, submission_status: "draft" as ContractSubmissionStatus, submitted_at: null, process_status: "incomplete" as ContractStatus, is_process_visible: false, management_notes: []
  };
...
export async function PATCH(request: Request) {
  ...
  if (text(body,"process_status")) {
    ...
    Object.assign(pendingUpdates, { process_status: contract.process_status, is_process_visible: contract.is_process_visible, process_note: contract.process_note });
    if (nextStatus === "resigned") { contract.resignation_note = text(body,"note"); contract.resigned_at = new Date().toISOString(); Object.assign(pendingUpdates, { resignation_note: contract.resignation_note, resigned_at: contract.resigned_at }); }
  }
  ...
  if (!fallbackMode && Object.keys(pendingUpdates).length) {
    const { error } = await supabaseAdmin.from("contracts").update({ ...pendingUpdates, updated_at: new Date().toISOString() }).eq("id", id);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  }
