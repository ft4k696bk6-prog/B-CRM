  ...
  return <AppShell profile={profile}><div className="grid gap-5">
    ...
    <section className="app-card"><SectionHeader icon={FileSignature} title="Realizacja" />
      {admin ? <>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><div className="text-sm"><div>Termin montażu: <b>{contract.installation_scheduled ? formatDateTime(contract.installation_at) : "Nie umówiono"}</b>{contract.installation_scheduled && !isArchived(contract) ? <button className="ml-3 min-h-10 text-sky" disabled={workflow.busy} onClick={() => workflow.openSchedule(contract)}>Zmień termin</button> : null}</div>{contract.installation_scheduled ? <div className="mt-1">Monter: <b>{contract.installer_name || contract.installer?.full_name || "Nieprzypisany"}</b></div> : null}</div><ContractArchiveButton contract={contract} busy={workflow.busy} onArchive={workflow.openArchive} onRestore={workflow.restore} /></div>
