...
export function useContractWorkflowActions({ accessToken, onUpdated, onStale }: {
  ...
  function openSchedule(contract: ContractRecord) {
    ...
    setInstallerName(contract.installer_name || contract.installer?.full_name || "");
  ...
  const dialogs = <>
    <ModalShell open={Boolean(schedule)} title="Umów montaż" description={schedule?.customer_name} size="sm" onClose={() => !busy && setSchedule(null)}>
      <form onSubmit={async (event) => {
        ...
        if (!schedule || !date || !installerName.trim() || Number.isNaN(Date.parse(date))) return;
        if (await mutate(schedule, { action: "workflow", field: "installation_scheduled", value: true, installation_at: new Date(date).toISOString(), installer_name: installerName.trim() })) setSchedule(null);
        ...
        <label><span className="label">Monter / ekipa</span><input required className="field" value={installerName} onChange={(event) => setInstallerName(event.target.value)} placeholder="Wpisz dowolną nazwę, np. Jan Kowalski / Ekipa 2" maxLength={160} /></label>
        {error ? <Alert tone="danger">{error}</Alert> : null}
        <button className="btn-primary" disabled={busy || !date || !installerName.trim()}>{busy ? "Zapisywanie…" : "Zapisz termin montażu"}</button>
      </form>
    </ModalShell>
