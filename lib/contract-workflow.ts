...
export function validateWorkflowCommand(body: Record<string, unknown>): string | null {
  ...
  if (body.field === "installation_scheduled" && body.value === true) {
    ...
    const installerName = typeof body.installer_name === "string" ? body.installer_name.trim() : "";
    const legacyInstallerId = typeof body.installer_id === "string" ? body.installer_id.trim() : "";
    if (!installerName && !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(legacyInstallerId))
      return "Wpisz montera lub ekipę.";
    if (installerName.length > 160)
      return "Nazwa montera jest za długa.";
  }
  return null;
}
