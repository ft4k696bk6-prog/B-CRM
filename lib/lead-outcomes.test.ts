...
describe("lead outcomes", () => {
  ...
  it("offers the correct actions before and after a meeting", () => {
    expect(allowedOutcomes("Nowy")).toEqual(["callback", "meeting", "no_answer", "return"]);
    expect(allowedOutcomes("Spotkanie", "2026-09-02T09:00:00.000Z", now)).toEqual(["contract", "resignation", "callback", "return"]);
    expect(allowedOutcomes("Spotkanie", "2026-09-02T11:00:00.000Z", now)).toEqual(["callback", "meeting", "no_answer", "return"]);
  });
