...
-- Assign a specific installer to a scheduled installation.
alter table public.contracts
  add column if not exists installer_id uuid references public.profiles(id) on delete set null;

create index if not exists contracts_installer_idx
  on public.contracts(installer_id)
  where installer_id is not null;

-- Keep the installer assignment behind the same guarded workflow path as the
-- installation date and process status.
...
declare
  ...
  installer_profile public.profiles;
    ...
    case field_name
      ...
      when 'installation_scheduled' then
        ...
        if field_value then
          ...
          -- During rollout, older cached clients may omit installer_id. The new
          -- API requires it, while this RPC preserves the current assignment so
          -- the schema migration itself does not break an already open session.
          if p_command ? 'installer_id' then
            next_installer_id := nullif(p_command->>'installer_id','')::uuid;
            if next_installer_id is null then raise exception 'Wybierz montera.'; end if;

            select * into installer_profile
            from public.profiles
            where id = next_installer_id;

            if installer_profile.id is null
              or installer_profile.crm_environment is distinct from actor.crm_environment
              or installer_profile.role is distinct from 'monter' then
              raise exception 'Wybrany użytkownik nie jest monterem w tym CRM.' using errcode='42501';
            end if;
          end if;
        else
