...
alter table public.contract_workflow
...
-- Existing paid contracts necessarily passed invoicing, but we do not infer
-- historical commission payouts.
update public.contract_workflow
set invoice_issued = true
where settled = true and invoice_issued = false;
...
begin
  ...
  if operation = 'workflow' then
    if w.archived_at is not null and field_name <> 'commission_paid' then
      raise exception 'Najpierw przywróć umowę z archiwum.';
    end if;
    ...
    case field_name
      ...
      when 'invoice_issued' then w.invoice_issued := field_value;
      when 'settled' then
        w.settled := field_value;
        if not field_value then w.commission_paid := false; end if;
      when 'commission_paid' then
        if field_value and not w.settled then
          raise exception 'Najpierw oznacz umowę jako rozliczoną.';
        end if;
        w.commission_paid := field_value;
  ...
  update public.contract_workflow set
    ...
    invoice_issued=w.invoice_issued,
    settled=w.settled,
    commission_paid=w.commission_paid,
  ...
  update public.contracts set
    ...
    process_status=case
      ...
      when w.commission_paid then 'commission_paid'
      when w.settled then 'settled'
      when w.invoice_issued then 'invoice_issued'
      when c.submission_status='draft' then 'incomplete'
      when w.installation_scheduled then 'installation_scheduled'
      when w.equipment_ordered then 'equipment_ordered'
      else 'verification'
    end,
    resigned_at=case
